...
class DiagramLatexCompiler:
    ...
    def compile_latex(self, latex_code: str, output_format: str = "png") -> Tuple[bool, Optional[bytes], Optional[str]]:
        ...
        """
        try:
            print(f"DiagramCompiler: Starting compilation to {output_format}")
            ...
            with tempfile.TemporaryDirectory(dir=self.temp_dir) as temp_dir:
                ...
                if output_format == "pdf":
                    print("DiagramCompiler: Returning PDF content")
                    try:
            ...
            error_str = str(e)
            print(f"DiagramCompiler: Exception occurred: {error_str}")
            return False, None, f"Compilation failed: {error_str}"
    
    def _compile_with_tectonic(self, tex_file: Path) -> Tuple[bool, Optional[str]]:
        """Try to compile with Tectonic LaTeX engine - optimized for TikZ"""
        try:
            print("DiagramCompiler: Trying Tectonic...")
            ...
            for path in tectonic_paths:
                print(f"DiagramCompiler: Checking Tectonic path: {path}")
                try:
                    ...
                    print(f"DiagramCompiler: Tectonic not found at {path}: {e}")
                    continue
            ...
            try:
                # Set up environment for Tectonic - disable fontconfig to avoid font issues
                env = os.environ.copy()
                env['FONTCONFIG_FILE'] = ''
                env['FONTCONFIG_PATH'] = ''
                env['TECTONIC_MINIMAL_MODE'] = '1'
                
                print(f"DiagramCompiler: Running {tectonic_cmd} on {tex_file.name}")
                ...
                print(f"DiagramCompiler: Tectonic return code: {result.returncode}")
                ...
                print("DiagramCompiler: Tectonic compilation successful")
                return True, None
    ...
    def _compile_with_pdflatex(self, tex_file: Path) -> Tuple[bool, Optional[str]]:
        ...
        try:
            print("DiagramCompiler: Trying pdflatex as fallback...")
            ...
            for path in pdflatex_paths:
                try:
                    ...
                    if result.returncode == 0:
                        pdflatex_cmd = path
                        print(f"DiagramCompiler: Found pdflatex at: {path}")
                        break
            ...
            try:
                print(f"DiagramCompiler: Running {pdflatex_cmd} on {tex_file.name}")
                result = subprocess.run(
                ...
                print(f"DiagramCompiler: pdflatex return code: {result.returncode}")
                ...
                print("DiagramCompiler: pdflatex compilation successful")
                return True, None
    ...
    def _create_complete_document(self, latex_code: str) -> str:
        """Create a complete LaTeX document optimized for TikZ diagrams"""
        print("DiagramCompiler: Creating complete document for diagram...")
        print(f"DiagramCompiler: Input LaTeX code length: {len(latex_code)} characters")
        
        # Check if the input already contains documentclass - if so, use as-is
        if '\\documentclass' in latex_code:
            print("DiagramCompiler: Input appears to be a complete document")
            return latex_code
        
        # If the input is empty or just whitespace, create a minimal document
        if not latex_code.strip():
            print("DiagramCompiler: Input is empty, creating minimal document")
...
\\begin{center}
\\textit{No diagram content provided}
\\end{center}

\\end{document}"""
        
        # Document header optimized for TikZ diagrams
...
\\usetikzlibrary{shapes.geometric, arrows, positioning, backgrounds, fit, calc, decorations.pathmorphing, decorations.markings}

\\geometry{margin=0.5in}

% Define common colors for diagrams
...
"""
        ...
        print(f"DiagramCompiler: Final document length: {len(result)} characters")
        
        return result
    
    def _pdf_to_png(self, pdf_file: Path) -> Tuple[Optional[bytes], Optional[str]]:
        """Convert PDF to PNG using pdf2image or fallback methods"""
        try:
            print("DiagramCompiler: Converting PDF to PNG...")
            
            if PDF2IMAGE_AVAILABLE:
                print("DiagramCompiler: Using pdf2image for conversion")
                ...
                for path in poppler_paths:
                    if os.path.exists(path) and os.path.exists(os.path.join(path, "pdftoppm.exe")):
                        poppler_path = path
                        print(f"DiagramCompiler: Found working Poppler at: {path}")
                        break
                
                if not poppler_path:
                    print("DiagramCompiler: No working Poppler installation found, trying without path")
                    try:
                        images = convert_from_path(str(pdf_file), dpi=300, first_page=1, last_page=1)
                    except Exception as e:
                        print(f"DiagramCompiler: pdf2image failed without path: {str(e)}")
                        return None, f"Poppler not found. Please install Poppler or add it to PATH. Error: {str(e)}"
                else:
                    print(f"DiagramCompiler: Using Poppler path: {poppler_path}")
                    images = convert_from_path(str(pdf_file), dpi=300, first_page=1, last_page=1, 
                ...
                if images:
                    ...
                    print("DiagramCompiler: PDF to PNG conversion successful with pdf2image")
            ...
            print(f"DiagramCompiler: PDF to PNG conversion failed: {str(e)}")
    ...
    def _pdf_to_png_external(self, pdf_file: Path) -> Tuple[Optional[bytes], Optional[str]]:
        ...
        try:
            ...
            result = subprocess.run(
                ['magick', 'convert', '-density', '300', str(pdf_file), str(png_file)],
                capture_output=True,
                text=True,
                timeout=self.timeout
            )
            ...
            if result.returncode != 0:
                ...
                result = subprocess.run(
                    ['pdftoppm', '-png', '-r', '300', str(pdf_file), str(pdf_file.stem)],
                    capture_output=True,
                    text=True,
                    timeout=self.timeout
                )
    ...
    def _create_mock_pdf(self, tex_file: Path) -> Tuple[bool, Optional[str]]:
        ...
        try:
            print("DiagramCompiler: Creating mock PDF for development...")
            
            # Read the LaTeX content
            with open(tex_file, 'r', encoding='utf-8') as f:
                latex_content = f.read()
            
            # Create a proper PDF structure with diagram-specific content
...
/F1 18 Tf
50 750 Td
(Diagram LaTeX Helper - Preview Mode) Tj
0 -40 Td
/F1 14 Tf
(TikZ Diagram Content Detected) Tj
0 -30 Td
/F1 12 Tf
(This is a preview generated in development mode.) Tj
0 -20 Td
(Your diagram LaTeX content was processed successfully.) Tj
0 -40 Td
/F1 10 Tf
(For production use, ensure Tectonic is properly configured) Tj
0 -15 Td
(with TikZ support to get full diagram compilation.) Tj
0 -40 Td
/F1 8 Tf
(LaTeX source length: {len(latex_content)} characters) Tj
0 -15 Td
(Contains TikZ: {"Yes" if "tikzpicture" in latex_content.lower() else "No"}) Tj
ET
endstream
endobj
...
%%EOF"""
            ...
            print("DiagramCompiler: Enhanced mock PDF created successfully")
            return True, None
            
        except Exception as e:
            print(f"DiagramCompiler: Failed to create mock PDF: {str(e)}")
    ...
    def _create_mock_png(self, pdf_file: Path) -> Tuple[Optional[bytes], Optional[str]]:
        ...
        try:
            print("DiagramCompiler: Creating mock PNG for development...")
            
            # Create a simple PNG header and basic image data for diagrams
            png_data = bytearray([
                0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A,  # PNG signature
                0x00, 0x00, 0x00, 0x0D, 0x49, 0x48, 0x44, 0x52,  # IHDR chunk
                0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0xC8,  # Width: 256, Height: 200
                0x08, 0x02, 0x00, 0x00, 0x00, 0x4C, 0x8D, 0x2E, 0x37,  # Bit depth, color type, etc.
            ])
            
            # Add basic image data for diagram mockup
            png_data.extend([
            ...
            print("DiagramCompiler: Mock PNG created successfully")
            return bytes(png_data), None
            
        except Exception as e:
            print(f"DiagramCompiler: Failed to create mock PNG: {str(e)}")
            return None, f"Failed to create mock PNG: {str(e)}"

# Global compiler instance
diagram_compiler = DiagramLatexCompiler()